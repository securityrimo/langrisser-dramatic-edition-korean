...
def convert(
    ...
    if unresolved:
        raise RuntimeError(
            "The v1.1.2 package MDF SHA-256 value is unresolved: "
            + ", ".join(unresolved)
        )
    ...
    if actual_hash.lower() != EXPECTED_PATCHED_SHA256:
        raise ValueError(
            "This is not the verified v1.1.2 patched MDF.\n"
...
def main() -> int:
    ...
    parser.add_argument("source", type=Path, help="v1.1.2 patched MDF")
