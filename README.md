...
- v1.0 원클릭 패처가 정상 LDP를 이전 내부 후보의 해시로 검사해 “패치 파일 손상”으로 거부하던 문제 수정
- v01321 게임 데이터와 패치 레코드는 그대로 유지하고 LDP manifest와 패처 검증값을 v1.1로 다시 맞춤
...
수동 적용용 LDP는 원클릭 ZIP에 포함되지 않으며, GitHub 릴리스의 별도 자산 `langrisser_de_ko_v1.1.ldp`로 제공됩니다. 저장소의 `apply_patch.py`, `make_cue_bin.py`와 이 LDP를 같은 폴더에 둔 뒤 Python 3.8 이상에서 실행합니다.

```powershell
python apply_patch.py "D:\경로\langDramaticEdition.mdf" "D:\경로\langDramaticEdition_ko_v1.1.mdf"
python make_cue_bin.py "D:\경로\langDramaticEdition_ko_v1.1.mdf" "D:\경로\Langrisser_Dramatic_Edition_Korean_v1.1.bin"
```

두 번째 명령이 만드는 `Langrisser_Dramatic_Edition_Korean_v1.1.cue`가 권장 MODE2/XA CUE입니다. 일반 사용자는 이 수동 방법을 사용할 필요가 없습니다.

## 검증 범위와 중요 사항

- v1.1의 공식 런타임 검수 환경은 `RetroArch 1.22.2 / Beetle Saturn` 코어이며, 런타임 확인 범위는 아래 용병 고용창 출력입니다. 검수한 v01321 QA MDF/BIN과 릴리스 목표 MDF/BIN은 바이트 단위로 같습니다.
